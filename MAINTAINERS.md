...
This document lists the people responsible for triaging issues, reviewing PRs, and cutting releases for `cycles-openai-agents` (the integration package that brings Cycles runtime authority into the OpenAI Agents SDK).
...
| Release cuts | Project lead | PyPI publish via `python-publish.yml` on tag |
| Dependency updates | Dependabot + project lead | Auto-merge on patch updates passing CI; manual review on minor/major |
| OpenAI Agents SDK compatibility | Project lead | Track upstream OpenAI Agents SDK releases; bump compatibility per major SDK change |
...
- **General questions / bug reports**: [GitHub Issues](https://github.com/runcycles/cycles-openai-agents/issues)
